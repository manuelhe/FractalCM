...
class NodeMap {
    ...
    /**
     *
     * Returns non inherited Content parameters
    ...
    public static function params($nodeId=FALSE) {
        ...
        $defaultParams = NodeParams::defaultParams();
        ...
        $params    = json_decode(base64_decode($r1));

        $ret    = array();
        while(list(,$v)=each($defaultParams)){
            $dVal    = isset($v['values']) && isset($v['values'][0]) && isset($v['values'][0]['val'])
                ? $v['values'][0]['val']
                : $v['values'];
            $ret[$v['param']] = isset($params->$v['param'])
                ? $params->$v['param']
                : $dVal;
        }
        return $ret;
    ...
    public static function children($nodeId=null,$visible=true,$completeData=false){
        ...
        $nav    = I18n::array_asoc_sort($nav,$params['sortBy'],$params['sortDir']);
        return $nav;
    }
    /**
     *
